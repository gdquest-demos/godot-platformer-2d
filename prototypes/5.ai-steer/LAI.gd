...
var _controller: BehaviorController2D
var _behavior: SteeringBehavior2D
var _arrive: ArriveBehavior2D
var _align: AlignBehavior2D
var _priority: PrioritySteering2D
var _flee: FleeBehavior2D
...
func _ready():
	...
	_arrive = get_node("BehaviorController2D/PrioritySteering2D/ArriveBehavior2D") as ArriveBehavior2D
	_arrive.target = _target
	
	_align = get_node("BehaviorController2D/PrioritySteering2D/BlendedBehavior2D/AlignBehavior2D") as AlignBehavior2D
	_align.target = _target
	
	_priority = get_node("BehaviorController2D/PrioritySteering2D") as PrioritySteering2D
	
	_flee = get_node("BehaviorController2D/PrioritySteering2D/BlendedBehavior2D/FleeBehavior2D")
	_flee.target = _target
	
	_controller = get_node("BehaviorController2D")
...
func _process(delta):
	...
	_behavior.calculate_steering(_motion)
