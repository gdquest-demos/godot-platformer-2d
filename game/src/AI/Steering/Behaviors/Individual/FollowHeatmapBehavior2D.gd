...
var _heatmap: HeatmapGD
var _last_point_index: int = INF
var _last_velocity: = Vector2(0, 0)


func _ready() -> void:
	var heatmap = get_tree().root.find_node("Heatmap", true, false)
	if heatmap is HeatmapGD:
		_heatmap = heatmap
	assert _heatmap
...
func _calculate_steering_internal(steering: SteeringMotion2D) -> SteeringMotion2D:
	...
	if point_index != _last_point_index:
		...
		var to_target: = _heatmap.best_direction_for(actor.global_position)
